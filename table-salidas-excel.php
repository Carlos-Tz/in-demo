...
if ($rubro['total'] > 0):
    ...
    endforeach;
$sheet->getStyle('G'.$fila.':H'.$fila)->getFont()->setBold(true)->setSize(12);
$sheet->getStyle('G'.$fila.':H'.$fila)->getFont()->getColor()->setRGB('FFFFFF');
$sheet->getStyle('G'.$fila.':H'.$fila)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('00ac69');

$sheet->setCellValue('G'.$fila, 'TOTAL: ');
$sheet->setCellValue('H'.$fila, $total_salidas);
